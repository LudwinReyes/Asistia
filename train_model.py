...
def train_recognition_model():
    try:
        ...
        if num_classes == 1:
            print(f"Solo 1 clase detectada: {class_names[0]}")
            print("Creando modelo simplificado para una sola persona...")
            
            # Create a simple classifier for single person
            classifier_model = FaceClassifier(embedding_dim=512, num_classes=1).to(device)
            
            # Save models
            torch.save(facenet_model.state_dict(), 'facenet_model.pth')
            torch.save(classifier_model.state_dict(), 'classifier_model.pth')
            
            # Create dummy SVM for compatibility
            from sklearn.dummy import DummyClassifier
            dummy_svm = DummyClassifier(strategy='constant', constant=0)
            dummy_svm.fit(train_embeddings[:2], train_labels[:2])
            joblib.dump(dummy_svm, 'svm_model.pkl')
            
            print(f"Embeddings guardados: {len(train_embeddings)} entrenamiento, {len(test_embeddings)} prueba")
            
        else:
            # Train SVM classifier (multiple classes)
